...
use crate::io::{IO_ALIGN, IO_BUFFER_ALLOCATOR};

use super::allocator::AlignedAllocator;

/// A capacity-fixed 4K-aligned u8 buffer.
pub struct IoBuffer {
    inner: BoxA<[u8], &'static AlignedAllocator<IO_ALIGN>>,
}
...
impl IoBuffer {
    ...
    pub fn new(size: usize) -> Self {
        let size = bits::align_up(IO_ALIGN, size);
        let mut v = VecA::with_capacity_in(size, &IO_BUFFER_ALLOCATOR);
        let aligned = bits::align_down(IO_ALIGN, v.capacity());
...
impl Deref for IoBuffer {
    type Target = BoxA<[u8], &'static AlignedAllocator<IO_ALIGN>>;
...
pub struct IoBytesMut {
    inner: VecA<u8, &'static AlignedAllocator<IO_ALIGN>>,
}
...
impl From<VecA<u8, &'static AlignedAllocator<IO_ALIGN>>> for IoBytesMut {
    fn from(value: VecA<u8, &'static AlignedAllocator<IO_ALIGN>>) -> Self {
        Self { inner: value }
    }
}
...
impl Deref for IoBytesMut {
    type Target = VecA<u8, &'static AlignedAllocator<IO_ALIGN>>;
...
impl IoBytesMut {
    ...
    pub fn align_to(&mut self) {
        let aligned = bits::align_up(IO_ALIGN, self.inner.len());
...
pub struct IoBytes {
    inner: Arc<BoxA<[u8], &'static AlignedAllocator<IO_ALIGN>>>,
    offset: usize,
    len: usize,
}
...
impl From<VecA<u8, &'static AlignedAllocator<IO_ALIGN>>> for IoBytes {
    fn from(mut value: VecA<u8, &'static AlignedAllocator<IO_ALIGN>>) -> Self {
        let offset = 0;
        let len = value.len();

        let aligned = bits::align_up(IO_ALIGN, value.len());
...
impl From<IoBuffer> for IoBytes {
    fn from(value: IoBuffer) -> Self {
        assert!(bits::is_aligned(IO_ALIGN, value.len()));
        let offset = 0;
        let len = value.len();
...
impl IoBytes {
    ...
    pub fn as_aligned(&self) -> &[u8] {
        ...
        let end = bits::align_up(IO_ALIGN, self.offset + self.len);
        debug_assert_eq!(start % IO_ALIGN, 0);
        debug_assert_eq!(end % IO_ALIGN, 0);
...
mod tests {
    ...
    fn test_io_bytes() {
        ...
        assert_eq!(buf.as_aligned().len(), IO_ALIGN);
